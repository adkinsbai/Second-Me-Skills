  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok || data?.code !== 0) {
        setError(
          data?.message ||
            (res.status >= 500 ? `服务器错误(${res.status})` : "登录失败")
        );
      } else {
  ...
  return (
    <main className="page-shell flex min-h-screen flex-col items-center justify-center px-4">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        />
        <div
          className="absolute left-1/4 top-2/3 h-64 w-64 rounded-full blur-3xl"
          style={{
            background:
              "radial-gradient(circle,rgba(255,45,109,0.06),transparent 70%)",
          }}
        />
      </div>
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <Link
                ...
                className="font-black text-[var(--love)] hover:underline"
              >
                忘记密码？
              </Link>
            </div>
